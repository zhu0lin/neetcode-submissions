...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        Update: Use a dictionary. Key=node, value=Node(node.val)
        Go through linked list, making the dictionary have above specifications
        Initialize new_head as Node(head.val)
        Go through linked list again
        Set node.next = node.next 
        Set node.random = node.random
        Return new_head
        """
        if not head:
            return None
        ...
        curr = head
        while curr:
            deep_copy[curr].next = deep_copy.get(curr.next) # get our current node's next node from the dictionary
